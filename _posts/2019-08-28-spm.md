...
During WWDC 2019 Apple announced that Swift Package Manager (SPM) now supports iOS projects. iOS developers have been waiting for a long time for this feature. Only Mac projects could benefit from SPM in the past. We have learned to use CocoaPods and Carthage but those tools are not perfect and SPM should allow us to replace any need in third-party dependency manager.

Many big companies separate their code into multiple internal frameworks and reuse them across their application. In the company I am working right now – [IG,][1] we have the same approach. I have decided to try SPM for managing our dependencies. 
